...
def validate(mod, census, today):
    ...
    for pid, e in S.items():
        ...
        # Chris, 14.09.2026: vitals[0] is the status line (fever state | pressors | O2), always.
        if not e["vitals"] or not status_line_ok(e["vitals"][0]):
            errors.append("%s: vitals[0] must be the status line '<afebrile since dd/mm or Tmax> | <no pressors / norad since dd/mm> | <RA / O2 / intubated + settings>': %r"
                          % (w, e["vitals"][0] if e["vitals"] else None))
        for v in e["vitals"][1:]:
            if ROUTINE_LAB.search(v) and not IMAGING.search(v) and not v.startswith("!"):
                warns.append("%s: routine lab dropped from the handout (prefix '!' if it made the diagnosis): %s" % (w, v))
