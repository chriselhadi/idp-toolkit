...
Compact format (set by Chris, 14.09.2026, "at least half the size"): the recap is the
handout's short form. Per patient: room + name, ID one-liner, then ONE line per issue
"n. dx (date); drug segs; drug segs. status", pending item names only. No Findings
line, no Labs line, no Micro line, no D-day counters, no drug notes, no gates: all of
that is on the handout. Inherit, do not rewrite: same text as yesterday unless the
synth entry changed. whatsapp.txt is this compact form; the verbose form is gone.
...
def rx_short(abx, today):
    """Drugs in start-date order, 'Drug [route] segs' only, no day counter and no note."""
    parsed = []
    for ab in abx:
        m = ABX_RE.match(ab)
        if not m:
            parsed.append(("9999", ab)); continue
        route = m.group("route") or ""
        head = m.group("drug") + ("" if route in ("", "IV") else " " + route)
        a, _b = seg_bounds(m.group("segs"), int(today[:4]))
        parsed.append((a or "9999", "%s %s" % (head, m.group("segs"))))
    parsed.sort()
    return "; ".join(t for _a, t in parsed)
...
def block(c, e, today, dropped):
    ...
    for i, iss in enumerate(e["issues"], 1):
        ...
        if m:
            L.append("%d. %s (%s)%s. %s" % (i, m.group("dx"), m.group("date"), rx, m.group("status")))
        else:
...
def block_verbose(c, e, today, dropped):
