...
def finalize(state_p, synth_p, census_p, as_p, today, out_p):
    ...
    if as_today:
        ...
        state["as_snapshot"] = snap
        # as_history (14.09.2026): the last AS_HISTORY_DAYS lists, so that a delta can be
        # taken against the last list whose delta report was actually SENT, not merely the
        # last one consumed. A day with no AS list, or a run that built but never sent,
        # then rolls into the next report instead of silently disappearing.
        hist = [h for h in (state.get("as_history") or []) if h.get("list_date") != snap["list_date"]]
        hist.append(snap)
        hist.sort(key=lambda h: h.get("list_date") or "")
        state["as_history"] = hist[-AS_HISTORY_DAYS:]
